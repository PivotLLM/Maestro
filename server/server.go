...
func (s *Server) registerTools() error {
	...
	s.mcpServer.AddTool(
		s.defaultTool(global.ToolTaskSetCreate,
			...
			mcp.WithBoolean("skip_validation",
				mcp.Description("Skip schema validation and report generation for this task set (default: false)"),
			),
			mcp.WithString("callback_url",
				mcp.Description("URL to POST completion notification when tasks finish"),
			),
	...
	s.mcpServer.AddTool(
		s.defaultTool(global.ToolTaskSetUpdate,
			...
			mcp.WithString("skip_validation",
				mcp.Description("Set skip_validation: 'true' or 'false' (optional)"),
			),
			mcp.WithString("callback_url",
				mcp.Description("URL to POST completion notification when tasks finish (optional)"),
			),
		), s.handleTaskSetUpdate)

	s.mcpServer.AddTool(
		s.defaultTool(global.ToolTaskDispatch,
			mcp.WithDescription("Create and immediately execute a single task asynchronously. Returns immediately with task UUID and status 'running'. Optionally fires a callback when complete."),
			mcp.WithString("project",
				mcp.Description("Project name"),
				mcp.Required(),
			),
			mcp.WithString("path",
				mcp.Description("Taskset path (auto-generated as 'dispatch/<uuid8>' if omitted)"),
			),
			mcp.WithString("title",
				mcp.Description("Task title (default: 'Dispatched task')"),
			),
			mcp.WithString("llm_model_id",
				mcp.Description("LLM model ID to use"),
			),
			mcp.WithString("prompt",
				mcp.Description("Inline prompt text"),
			),
			mcp.WithString("instructions_text",
				mcp.Description("Multi-line instructions text"),
			),
			mcp.WithString("instructions_file",
				mcp.Description("Instructions file path"),
			),
			mcp.WithString("instructions_file_source",
				mcp.Description("Source for instructions file: project, playbook, or reference"),
			),
			mcp.WithString("callback_url",
